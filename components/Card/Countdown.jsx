    ...
    const loadVotingPeriod = async () => {
      ...
      try {
        ...
        if (votingPeriod?.start && votingPeriod?.end) {
          setEventStartDate(
            new Date(votingPeriod.start * 1000).toISOString().slice(0, 16)
          );
          setEventEndDate(
            new Date(votingPeriod.end * 1000).toISOString().slice(0, 16)
          );
  ...
  const handleSubmit = async (e) => { 
    ...
    setCountdownStarted(true);
     if (!currentAccount) {
         ...
         await setVotingPeriod(startTimestamp, endTimestamp);
         setVotingAlreadySet(true);  // <== SET IT TRUE AFTER SUBMITTING
