...
const Archives = () => {
  ...
  const handleArchive = async () => {
    try {
      ...
      const convertedVoteCounts = sessionData.voteCounts.map(count => {
        // Check if it's a BigNumber object
        if (count && typeof count === 'object' && count.toString) {
          return count.toString();
        }
        return String(count); // Ensure it's a string regardless
      });
      
      // Calculate total votes from the converted values
      const totalVotes = convertedVoteCounts.reduce((sum, count) => sum + parseInt(count || 0), 0);
      
      // Convert winnerVoteCount to string if it's a BigNumber
      const winnerVoteCount = sessionData.winnerVoteCount && 
        typeof sessionData.winnerVoteCount === 'object' && 
        sessionData.winnerVoteCount.toString ? 
        sessionData.winnerVoteCount.toString() : 
        String(sessionData.winnerVoteCount || 0);
      ...
      const newArchive = {
        ...
        details: {
          ...
          voteCounts: convertedVoteCounts, // Use the converted array
          winnerAddress: sessionData.winnerAddress,
          winnerVoteCount: winnerVoteCount, // Use the converted value
          startTime: sessionData.startTime,
          endTime: sessionData.endTime
        }
      };

      console.log("Prepared archive data:", JSON.stringify(newArchive, null, 2));
